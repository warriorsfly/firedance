...
2. OpenOCD: Starts a debug session for a `32F429IDISCOVERY` board (or any `stm32f429i` running at 8MHz).
...
For example, the 32F429IDISCOVERY board uses an mcu from the `stm32f429i` line of processors.  
All the SVD files for the STM32F3 series are available on [ST's Website][stm32f3].  
Download the [stm32f3 SVD pack][stm32f3-svd], and copy the `STM32F429.svd` file into `.vscode/`.  
